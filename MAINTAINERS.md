# MAINTAINERS.md — labs-secure-ai (template maintenance)

Maintainer conventions for the **template repo itself** — not for student copies. (`CLAUDE.md` is
deliberately student-facing: it ships into every copy and becomes the student agent's standing
instructions, so nothing maintainer- or estate-specific may live there.)
...
template flow ("Include all branches," public) and run the workshop's labs against their own copy.
...
5. **Course alignment beats local cleverness.** A change to staged content that drifts from the
   ...
   change is right.
6. **When `main` changes, rebase the three staged branches onto it** (force-push is fine on the
   non-default staged branches). A stale base pollutes the students' staged-PR diffs with
   unrelated file changes — the diff against `main` must show exactly the planted fixtures,
   nothing else. Verify with a three-dot diff against `main` after every rebase.
7. **Keep `CLAUDE.md` student-safe.** Anything you'd want an agent to know while maintaining the
   template belongs here, not there.
